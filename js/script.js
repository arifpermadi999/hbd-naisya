...
function startSoundWithUserGesture() {
    ...
    if (soundManager.ctx.state === 'suspended' || soundManager.ctx.state === 'interrupted') {
    ...
    // 3. (Optional) Remove the button after it's been used
    const button = document.getElementById('start-sound-button');
    if (button) {
        button.remove();
    }
}